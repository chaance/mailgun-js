import { urlJoin, btoa } from "./utils";
import APIError from "./error";
import type { RequestOptions, APIErrorOptions } from "./types";
import type NodeFormData from "form-data";
import type nodeFetch from "node-fetch";
import type ky from "ky-universal";

const isStream = (attachment: any) =>
	typeof attachment === "object" && typeof attachment.pipe === "function";
...
class Request {
	...
	private fetch: typeof global.fetch | typeof nodeFetch | typeof ky;

	constructor(
		options: RequestOptions,
		formData: new () => FormData | NodeFormData
	) {
	...
	async request(method: string, url: string, options?: any) {
		...
		if (
			options?.query &&
			Object.getOwnPropertyNames(options?.query).length > 0
		) {
			params.searchParams = options.query;
			delete params.query;
		}

		const response = await this.fetch(urlJoin(this.url!, url), {
		...
		if (!response?.ok) {
			const message =
				response?.body && isStream(response.body)
