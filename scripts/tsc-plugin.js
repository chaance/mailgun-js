// https://github.com/ReactTraining/react-router/blob/v6.0.0-beta.0/scripts/rollup/tsc-plugin.js
		...
		async resolveId(id, importer) {
			if (!importer && /^packages\//.test(id) && /\.tsx?$/.test(id)) {
				...
				try {
					...
					let source = await fs.readFile(dtsFile, "utf-8");
		...
		async load(id) {
			if (id.startsWith(tscOutputDir)) {
				try {
					...
					let map = await fs.readFile(id + ".map", "utf-8");
